...
> 把 OpenAI 的 **Codex Micro** 核心功能，复刻到一台普通 **15 键 Stream Deck**（5×3）上：实时 Agent 状态、批准/拒绝、发送、新建对话、推理等级、按住说话。
...
![硬件预览](assets/preview/Hardware-preview.jpg)
...
| PTT | Send | 切换页面 | 上个对话 | 下个对话 |

| 行 | 动作 |
| --- | --- |
| 第 1 行 —— 对话键 | 实时对话键（插件动作） |
| 第 2 行 —— 命令键 | ⌘⌥A / ⌘⌥R / 打开 ChatGPT 应用 / ⌘N / ⌘⌥E |
| 第 3 行 —— 控制键 | ⌥Space / ⌘⌥S / 切换下一个 Profile / ⇧⌘[ / ⇧⌘] |
...
- **PTT**——用配置的快捷键（默认为左 Option + 空格）触发你自己的第三方按住说话服务（如微信语音输入）。目前只支持 **arm64 的 macOS**
- **上个对话 / 下个对话**——桌面端里的对话切换

> Profile 里绑定的快捷键并非 ChatGPT 应用的默认快捷键，如果需要使用插件里附带的 Profile，建议按照布局图所示修改默认的动作快捷键
（或者在 Stream Deck 官方软件中自行修改 Profile，修改为你当前使用的快捷键）
